...
const fileImportString = (componentName: string, path: string, exportType: "named" | "default", lazy = false) => {
    if (lazy && exportType === "named") return `const Lazy${componentName} = React.lazy(() => import("${path}").then(module => { console.log(module); return { default: module.${componentName} } }));`;
    if (lazy && exportType === "default") return `const Lazy${componentName} = React.lazy(() => import("${path}"));`;
    if (exportType === "named") return `import { ${componentName} } from "${path}";`;
...
export const compileFolderToSchema = (folderPath: string, routerPath = folderPath): FolderSchema => {
    ...
    for (const itemName of dirContent) {
        ...
        const exportType = getFunctionExportType(componentName, rawFile);
        const relativePath = path.relative(routerPath, folderPath).replace(/\\/g, "/").replace(/@/g, ":").replace(/\/\$/g, "/").replace(/\$/g, "/") || "/";
        const itemSchema = { path: itemPath.replace(/\\/g, "/"), fileName: itemName, componentName, realtivePath: relativePath, importString: "", export: exportType };

        itemSchema.importString = fileImportString(itemSchema.componentName, itemSchema.path, exportType);
        if (type === "lazy.page") {
            files["page"] = itemSchema;
            files["lazy.page"] = { ...itemSchema, path: `virtual:react-file-router/${crypto.createHash("sha256").update(itemSchema.path, "utf-8").digest("hex")}.tsx` };
            files["lazy.page"].importString = fileImportString(files["lazy.page"].componentName, files["lazy.page"].path, files["lazy.page"].export, true);
        } else {
